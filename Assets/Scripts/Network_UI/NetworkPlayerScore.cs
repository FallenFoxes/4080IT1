...
{
player.GetComponent<NetwrokPlayerData>().playerName.OnValueChanged +=
OnNameChanged;
player.GetComponent<NetwrokPlayerData>().score.OnValueChanged +=
OnScoreChanged;
player.GetComponent<NetwrokPlayerData>().playerNumber.OnValueChanged +=
OnplayerNumChanged;
OnScoreChanged(0, player.GetComponent<NetwrokPlayerData>().score.Value);
OnNameChanged("",
player.GetComponent<NetwrokPlayerData>().playerName.Value);
OnplayerNumChanged(0,
player.GetComponent<NetwrokPlayerData>().playerNumber.Value);
}
public void OnplayerNumChanged(ulong previousValue, ulong newValue)
{
playerNum.text = newValue.ToString();
}
public void OnNameChanged(FixedString128Bytes previousValue,
